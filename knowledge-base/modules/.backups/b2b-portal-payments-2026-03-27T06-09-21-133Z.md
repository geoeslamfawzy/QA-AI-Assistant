...
## Discovered: What is adjusted trips

> Added from Jira search on 2026-03-27
> Source tickets: CMB-35952: Replace TVA with TVA LF 2026 for algerian invoices, CMB-36581: Verify no adjusted invoice is generated when no adjusted trips, CMB-36580: Verify adjusted invoice is generated only when adjusted trips exist, CMB-35123: [iOS]Review and Fix Trip Estimation and Trip Request Events in  Firebase and Adjust, CMB-34802: EDGE - WEBAPP/ADMINPANEL - PROD : Adjusted trips price is set to positive values on the export files, CMB-35207: [DashOps] Refund Hybrid Payment Wallet Amount When Updating Trip Status, CMB-31489: [WEBAPP] B2B giftcard Trips History and remaining budget on webApp, CMB-17550: [Backend] - Blocking rules - Counting on Displayed assignments rather than sent ones, CMB-18770: MongoServerError: Caused by :: Write conflict during plan execution and yielding is disabled, CMB-35075: [Android]Review and Fix Trip Estimation and Trip Request Events in  Firebase and Adjust, CMB-35495: [Prod] Online archive & yassir-go-production cluster, CMB-33855: Same update on scheduling trip – Driver App (UX adjustments), CMB-34607: [Backend] Change DB Schema To Adapt The Streak Challenge, CMB-32002: Rider App - Align Automated Scheduling Design with Activity Design, CMB-34740: EDGE-ADMINPANEL-PROD : Duplicated adjusted trip invoice reference no consumption invoice ref (January)

## What are Adjusted Trips?

Based on the tickets provided, **adjusted trips** are a specific category of trips within the B2B/Enterprise billing context that are separated from regular "finished" trips for invoicing purposes.

Here is what the tickets reveal:

**Billing and Invoicing**

- Adjusted trips have their own dedicated invoice, distinct from the standard consumption invoice (CMB-34740 references "no consumption invoice ref" alongside an "adjusted trip invoice reference").
- According to CMB-36580, an **adjusted trips invoice is generated only when adjusted trips exist** within a billing period, and it **includes adjusted trips only** — they are not mixed with regular finished trips.
- According to CMB-36581, if an enterprise has **no adjusted trips**, no adjusted invoice should be generated at all.
- Per CMB-35952, both "finished trips invoices" and "adjusted trips invoices" are distinct invoice types for Algerian enterprises, both subject to TVA LF 2026 display rules.

**Pricing Behavior**

- According to CMB-34802, there was a production bug where **adjusted trips prices were set to positive values** in export files — implying that adjusted trips are expected to carry **negative values** (i.e., corrections or deductions).

**What Remains Unclear**

The tickets do not explicitly define the business rule that causes a trip to be classified as "adjusted" in the first place (e.g., price corrections, cancellations after billing, disputes). The tickets only describe how adjusted trips are handled in invoicing and exports.

In summary: adjusted trips appear to be **post-billing trip price corrections** for B2B enterprises, tracked separately with their own invoice, and expected to have negative monetary values representing adjustments to previously billed amounts.
